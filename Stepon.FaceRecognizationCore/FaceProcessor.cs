    ...
    public class FaceProcessor : IDisposable
    {
        ...
        public void Dispose()
        {
            if(_locator !=null){_locator.Dispose();}
              if(_recognize !=null){_recognize.Dispose();}
        ...
        public Feature[] LocateExtract(Bitmap image, LocateOperation operation = LocateOperation.None)
        {
            ...
            var code = _locator.Detect(image, out location, operation);
        ...
        {
            LocateResult location;
            var code = _locator.Detect(imageData, width, height, out location, pixelSize, operation);
            if (code != ErrorCode.Ok)
                throw new FaceException(code);
